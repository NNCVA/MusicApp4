...
主要页面和核心播放/媒体库流程已完成开发并持续优化，测试基础设施已完备。当前页面行为以 `app/` 下代码、自动化测试和实际设备验证为准；早期视觉对照图、页面 PRD 和实施计划仅用于历史追溯，不再作为开发门槛。

## 核心功能

- 媒体库扫描支持 `.mp3`、`.flac`、`.wav`、`.aac`、`.m4a`、`.ogg`、`.opus`，可组合 Android 媒体库与自定义文件夹；支持通过 SAF 选择外置存储与 U 盘文件夹并完成大小写兼容映射；提供包含/排除路径、屏蔽文件夹、静默增量与全量同步，并可跳过 60 秒以下音频。
- 提供单曲、专辑、艺术家、文件夹、播放列表和播放历史浏览；单曲列表支持手势下拉静默全量同步，清理失效曲目并提供增删状态细分反馈；健全外置存储设备生命周期管理，拔出断开后自动过滤隐藏，歌单保留元信息展示与离线防呆提示；支持多艺术家拆分与聚合、专辑版本/发行年份排序、文件夹层级与存储卷容量信息。
- 提供独立歌曲搜索页，可在全库、指定歌单或播放历史中按标题、艺术家、专辑检索，并结合排序、分组索引和多选操作。
- 支持歌单创建、重命名、删除、详情管理、TXT 导入/导出，以及批量加入队列、下一首播放、加入歌单、隐藏和歌曲信息查看。
- 使用单一 ExoPlayer、MediaLibraryService 与 MediaSession 实现后台播放、系统媒体控制、播放快照、列表循环/单曲循环/随机播放和稳定队列；优化物理时间线映射，消除切换播放模式时的音频中断卡顿。
