...
5. **播放模式**：随机开关与循环状态独立；默认随机关闭、循环关闭。单曲循环只影响自然结束，手动上一首/下一首仍切换队列；列表循环在队尾回到队首。随机序列生成后持久化，关闭随机恢复原队列并保留当前曲目。
...
14. **播放队列**：实时查看当前播放队列，支持拖拽重排、移除及点击跳转；随机模式禁用拖拽，移除当前曲目后切到下一首。
...
- 已接受的测试策略见 `docs/design-review-09-security-and-testing.md`；已搁置的性能指标不得作为当前发布阻断条件。

## 架构与设计参考文档 (设计规范提示词)
- 总索引：`docs/design-review-index.md`
- 领域词汇：`docs/CONTEXT.md`
- 架构决策：`docs/adr/`
- `docs/design-review-01` 至 `09` 以及 `11` 为已接受的首版实现约束。
- `docs/design-review-10-performance-release-questions.md` 已搁置，仅保留备查。
