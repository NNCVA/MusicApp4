...
- `media/service` 在导出的 MediaLibraryService 内唯一创建并持有单 ExoPlayer 与 MediaLibrarySession；应用代码只消费平台无关的 `PlaybackControllerFacade`，Activity 在 `onStart/onStop` 连接并释放可重建的 MediaController，避免空闲 Service 被永久绑定；可信系统控制器仅获得已实现的浏览与基础播放白名单，其他外部控制器拒绝连接。
