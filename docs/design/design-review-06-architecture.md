...
- `media/service` 在导出的 MediaLibraryService 内唯一创建并持有单 ExoPlayer 与 MediaLibrarySession；Service 内的队列协调器是播放模式、原始队列、稳定随机序列与 Player timeline 的单一状态源。应用代码只消费平台无关的 `PlaybackControllerFacade`，Activity 在 `onStart/onStop` 连接并释放可重建的 MediaController；队列与模式命令只授予同包同 UID 的应用控制器，可信系统控制器仅获得浏览与基础播放白名单，其他外部控制器拒绝连接。
