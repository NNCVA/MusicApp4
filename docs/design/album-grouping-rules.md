...
graph TD
    ...
    B -- 是 --> F["专辑主艺术家 = 该创作者名称"]
    B -- 否 --> E{"该创作者是否参与了每首曲目的创作署名?"}
    E -- 是 --> F
    E -- 否 --> D["判定为群星 (Various Artists)"]
```

- **门限说明**：
  - 判定准则：若存在主创作者满足以下**任一**条件，则专辑主艺术家认定为该创作者，否则归入群星 `<various_artists>`：
    1. 参演曲目数严格超过专辑总曲目数的 50%（`maxCount * 2 > stableTracks.size`）；
    2. 参与了专辑内的每一首曲目创作署名（`primaryArtistInEveryCredit == true`）。
  - 仅当上述两项条件均不满足时（即占比 <= 50% 且未能全勤参演），专辑艺术家才归为群星 `<various_artists>`，双语本地化展示为“群星”/“Various Artists”。
