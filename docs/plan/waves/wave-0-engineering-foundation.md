...
| ID | 状态 | 交付结果 | 依赖 | 最窄验证 |
|---|---|---|---|---|
| W0-01 | `TODO` | 版本目录与包边界 | 无 | Gradle 配置 + Debug 编译 |
| W0-02 | `TODO` | Hilt Application 与可替换系统源 | W0-01 | Hilt 编译测试 |
| W0-03 | `TODO` | Room/DataStore/Media3 基础接入 | W0-01、W0-02 | 空实现编译 + 配置检查 |
| W0-04 | `TODO` | 本地单元测试底座 | W0-01 | 空壳本地测试 |
| W0-05 | `TODO` | 设备、Compose 与截图测试底座 | W0-02、W0-04 | Runner 发现 + 设备冒烟 |
| W0-06 | `TODO` | 覆盖率、资源与架构检查任务 | W0-03～W0-05 | 目标 Gradle 任务可发现 |
| W0-07 | `TODO` | Release、备份、Manifest 与 R8 底座 | W0-03 | Release 配置及静态检查 |
...
W0-01～W0-07 全部 `DONE`；Debug/Release、空壳本地测试、设备冒烟、Lint 和质量任务均有可复查证据。没有设备时 Wave 0 保持 `BLOCKED`，不得把任务可发现等同于设备测试通过。
