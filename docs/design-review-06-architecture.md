...
1. **原子同步**：每次完整扫描分配同步代次，先 Upsert 本次发现曲目；只有查询和合并全部成功后才把未出现曲目标记为不可用，扫描失败不得删除或覆盖现有可用数据。
2. **失效曲目**：文件暂时离线、存储卷卸载或权限丢失时保留曲目及播放列表、历史关联，并在界面显示“不可用”；重新发现相同曲目标识后自动恢复，只有用户明确清理缓存才移除失效记录。
